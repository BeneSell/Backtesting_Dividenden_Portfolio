...
And also to visualize the results from the business logic classes
"""
...
class VisualizeFMP():
    ...
    def visualize_dividenden_data(self, stock_symbol:str):
        ...
        df_to_plot = self.preproccessed_data[\
                        ...
                        & (self.preproccessed_data["symbol"] == stock_symbol) ][["date", "value"]]
        ...
        df_to_plot.iplot(kind="bar",\
                        x="date",\
                        y="value",\
                        title=f"Datetime plot '{stock_symbol}' from financal modeling prep",\
                        xTitle="date",\
                        yTitle="dividend",\
                        asFigure=True).write_html(f"../data/vis/{stock_symbol}_fmp_dividende.html")
    ...
    def visualize_stock_as_candlestick(self, stock_symbol:str):
        ...
        temp_df = self.preproccessed_data_stock[\
                        (self.preproccessed_data_stock["variable"]\
                         .isin(["open", "high", "low", "close"]))]
    ...
    def visualize_stock_data(self, stock_symbol:str):
        ...
        df_to_plot.iplot(kind="line",\
                         x="date",\
                         y="value",\
                         title=f"Datetime plot '{stock_symbol}' from financal modeling prep",\
                         xTitle="x",\
                         yTitle="x",\
                         asFigure=True).write_html(f"../data/vis/{stock_symbol}_fmp_stock.html")
...
class VisualizeAlphavantage():
    ...
    def visualize_stock_data(self, stock_symbol_list=None):
        ...
        df_with_selected_information = self.preproccessed_data[\
                                       self.preproccessed_data["information"]\
                                        .str.strip() == "adjusted close"]
        ...
        df_to_plot.iplot(kind="bar",\
                    ...
                    xTitle="date",\
                    yTitle="stock price",\
                    asFigure=True).write_html(f"../data/vis/{stock_symbol_list}_alpha_stock.html")
    ...
    def visualize_dividenden_data(self, stock_symbol_list = None):
        ...
        df_with_selected_information = df_with_selected_information\
                                    .groupby(by="date").mean(numeric_only=True).reset_index()
...
class VisualizeCombinedData():
    ...
    def fmp_vs_alpha(self, stock_symbol:str = "ADBE"):
        ...
        filtered_df = self.combined_data[\
                        self.combined_data["information"]\
                            .isin(['alpha_close', 'fmp_close'])]

        filtered_df = filtered_df[[stock_symbol, 'date', 'information']]

        # Pivot the DataFrame to have 'adbe' as columns
        pivot_df = filtered_df.pivot_table(index='date',\
            columns='information',\
            values=stock_symbol,\
            aggfunc='mean')\
        ...
        pivot_df.reset_index().iplot(kind="bar",\
             x="date",\
             y=['alphavantage', 'fmp'],\
             title=f"Datetime plot '{stock_symbol}'",\
             xTitle="date",\
             yTitle="dividenden",\
             asFigure=True).write_html(f"../data/vis/{stock_symbol}_alpha_vs_fmp.html")
        # plt.show()
    ...
    def alpha_stock_vs_alpha_dividend(self, stock_symbol:str = "ADBE"):
        ...
        pivot_df = filtered_df.pivot_table(index='date',\
                ...
                values=stock_symbol,\
                aggfunc='mean')\
                .rename(columns={'alpha_close': 'alpha_close', 'alpha_dividend': 'alpha_dividend'})
...
class VisualizeResultData():
    ...
    def visualize_scatter_plots(self):
        ...
        """
        self.result_data.iplot(kind="bar",\
            x="symbol",\
            y="money_made",\
            title="Top 30 symbols",\
            xTitle="Symbol",\
            yTitle="Money made (AVG)",\
            asFigure=True).write_html("../data/vis/results_symbol_vs_money_made.html")
        self.result_data.iplot(kind="scatter",\
            x="time_span",\
            y="money_made",\
            mode="markers",\
            xTitle="Time span",\
            ...
            asFigure=True).write_html("../data/vis/results_time_span_vs_money_made.html")
        self.result_data.iplot(kind="scatter",\
            ...
            asFigure=True).write_html("../data/vis/results_look_forward_years_vs_money_made.html")
        self.result_data.iplot(kind="scatter",\
            ...
            asFigure=True).write_html("../data/vis/results_look_backward_years_vs_money_made.html")
        ...
        self.result_data.iplot(kind="scatter",\
            ...
            asFigure=True).write_html("../data/vis/results_all_vs_money_made.html")
        self.result_data.drop_duplicates(subset=["symbol","money_made"]).iplot(kind="scatter",\
            x="all",\
            y="money_made",\
            mode="markers",\
            xTitle="All",\
            yTitle="Money made",\
            title="Money made vs all", asFigure=True)\
        ...
        self.result_data.iplot(kind="scatter",\
            ...
            xTitle="Rank growth",\
            yTitle="Money made",\
            title="Money made vs rank growth",\
            asFigure=True)\
                .write_html("../data/vis/results_rank_growth_vs_money_made.html")
        self.result_data.iplot(kind="scatter",\
            x="rank_stability",\
            y="money_made",\
            mode="markers",\
            xTitle="Rank stability",\
            yTitle="Money made",\
            title="Money made vs rank stability",\
            asFigure=True)\
                .write_html("../data/vis/results_rank_stability_vs_money_made.html")
        self.result_data.iplot(kind="scatter",\
            x="rank_yield",\
            y="money_made",\
            mode="markers",\
            xTitle="Rank yield",\
            yTitle="Money made",\
            title="Money made vs rank yield",\
            asFigure=True)\
                .write_html("../data/vis/results_rank_yield_vs_money_made.html")
        self.result_data.iplot(kind="scatter",\
            x="yield",\
            y="money_made",\
            mode="markers",\
            xTitle="Yield",\
            yTitle="Money made",\
            title="Money made vs yield",\
            asFigure=True)\
                .write_html("../data/vis/results_yield_vs_money_made.html")
        self.result_data.iplot(kind="scatter",\
            x="growth",\
            y="money_made",\
            mode="markers",\
            xTitle="Growth",\
            yTitle="Money made",\
            title="Money made vs growth",\
            asFigure=True)\
                .write_html("../data/vis/results_growth_vs_money_made.html")
        self.result_data.iplot(kind="scatter",\
            x="stability",\
            y="money_made",\
            mode="markers",\
            xTitle="Stability",\
            yTitle="Money made",\
            title="Money made vs stability",\
            asFigure=True)\
                .write_html("../data/vis/results_stability_vs_money_made.html")
    ...
    def visualize_vs_msiw(self, combined_data: pd.DataFrame):
        ...
        list_msciworld = [{"money_made": local_single_stock_checker\
                           .check_money_made_by_div(start_date=pd.to_datetime(start_date) \
                            + timedelta(days = 365 * (look_backward_years)),\
                                look_foward_years=x,\
                                symbol="MSCI",\
                                df_combined=combined_data,\
                                money_invested=100).iloc[-1]["money"],\
                                "date": pd.to_datetime(start_date)\
                                    + timedelta(days = 365 * look_backward_years) \
                                    + timedelta(days = 365 * x)} for x in range(3, 11)]
        ...
        portfolio_progression = self.result_data[\
                (self.result_data["time_span"] == year_selection)\
                & (self.result_data["look_backward_years"] == look_backward_years)]\
                    .groupby("future_date").mean(numeric_only=True)\
        ...
        msci_plot = go.Scatter(mode="lines",\
        ...
        fig.update_layout(title=f"msci world vs portfolio from"\
                            f"{year_selection+look_backward_years+1990 + 3}"\
                            f"to {year_selection+ 1990 + 3 + 7 + look_backward_years} years")

        fig.write_html("../data/vis/msci_world_vs_portfolio_"\
                       f"{year_selection+ look_backward_years+ 1990 + 3}"\
                       f"_{year_selection+ 1990 + look_backward_years + 3 + 7}.html")

    def visualize_histogram_plots(self):
        """
        This function is used to visualize the histogram plots from the result data
        """
        self.result_data[["money_made"]].iplot(kind="histogram",\
                x="money_made",\
                xTitle="Money made",\
                yTitle="Frequency",\
                title="Money made histogram",\
                asFigure=True).write_html("../data/vis/histogram_money_made.html")
        # histogram symbols sort by frequency
        self.result_data["symbol"].value_counts().iplot(kind="bar",\
                xTitle="Symbol",\
                yTitle="Frequency",\
                title="Symbol frequency",\
                asFigure=True).write_html("../data/vis/histogram_.html")

        # histogram all sort by frequency
        self.result_data["all"].value_counts().iplot(kind="bar",\
            xTitle="All",\
            yTitle="Frequency",\
            title="All frequency",\
            asFigure=True).write_html("../data/vis/histogram_.html")
        ...
        for x in range(1, 30):
            ...
            self.result_data[\
                        (self.result_data["time_span"] == x)]["money_made"].iplot(kind="histogram",\
                        x="money_made",\
                        xTitle="Money made",\
                        yTitle="Frequency",\
                        title="Money made histogram",\
    ...
    def visualize_symbol_vs_money_made(self):
        ...
        for x in range(1, 30):
                    ...
                    .groupby("symbol")\
                        .mean().reset_index()\
                            .iplot(kind="bar",\
                                ...
                                title="Money made histogram",\
                                asFigure=True)\
                                    .write_html(f"../data/vis/iteration_stuff/"\
            ...
            for y in range(3, 11):
                ...
                fig.add_trace(go.Bar(x=to_plot["symbol"],\
                            y=to_plot["money_made"],\
                            hovertext=to_plot["all"] ))

                fig.update_layout(title=f"Money made vs symbol timespan  Start_date:"\
                    f"{1990+x}, Middle_date {1990+x+y} "\
                    f"Future_date combined from {1990+x+y + 3} to {1990+x+y+10} years")
                fig.write_html("../data/vis/iteration_stuff/"\
    ...
    def visualize_symbol_vs_money_made_same_middle_date(self):
        ...
        but with the same middle date

        This means that the investment started at the same year
        """
            ...
            for x in range(3,11):
                ...
                start_date = self.result_data\
                    [(self.result_data["time_span"] == temp_year_selection)\
                     & (self.result_data["look_backward_years"] == temp_look_backward_years)]\
                        .groupby("future_date").first()["start_date"]
                if start_date.empty:
                    continue
                start_date = start_date.iloc[0]

                # middle_date = pd.to_datetime(start_date) \
                # + timedelta(days = 365 * temp_look_backward_years)
                # start_investment = 100
                ...
                fig.update_layout(title="Money made vs symbol,"\
                                  f"Year the Investment started:"\
                                  f"{1990+temp_year_selection + temp_look_backward_years}")
                for z in range(3,11):

                    to_plot = portfolio_progression\
                                [portfolio_progression["look_forward_years"] == z]



                    bar_chart = go.Bar(x=to_plot["symbol"],\
                        y=to_plot["money_made"],\
                        hovertext=to_plot["future_date"],\
                        name=f"Sold end of year:"\
                             f"{1990+temp_year_selection + temp_look_backward_years + z}")
                    fig.add_trace(bar_chart, secondary_y=False)


                fig.write_html("../data/vis/iteration_stuff/"\
                               f"symbol_vs_money_made_middle_date{1990 + middle_year}.html")
    ...
    def visualize_symbol_vs_money_made_same_future_date(self):
        ...
        some_df["together"] = some_df["time_span"] \
        ...
        for x in range(some_df["together"].min(), some_df["together"].max()):
            ...
            temp_df = some_df[\
                        (some_df["together"] == x)]\
                            .sort_values(by="time_span", ascending=False)
            ...
            bar_chart = go.Bar(x=temp_df["symbol"],\
    ...
    def visualize_portfolio_vs_money_made_same_future_date(self):
        ...
        for x in range(some_df["together"].min(), some_df["together"].max()):
            ...
            temp_df = some_df[(some_df["together"] == x)]\
                    .sort_values(by="time_span", ascending=False)
            ...
            temp_df = temp_df\
                    .groupby(by=["time_span"\
                        , "look_backward_years"\
                        , "look_forward_years"])\
                            ["money_made"].mean().reset_index()
    ...
    def visualize_portfolios_with_same_middledate(self):
        ...
        for y in range(11, 30):
            ...
            to_plot = to_plot.pivot_table(index="future_date",\
            ...
            for x in to_plot.columns:
                ...
                fig.add_trace(go.Scatter(mode="lines",\
                    ...
                    name=f"portfolio with {x}"\
                         ...
                         f"{1990 + middle_year}"), secondary_y=False)
            fig.write_html("../data/vis/"\
