...
class DataFrameTransformation(mn.Scene):
    ...
    def get_data(self):
        ...
        with open(
            "../data/stock_infos/raw_data_fmp_stock_value.json", "r"
        ) as json_file:
        ...
        pre_fmp = pre.PreproccessingFMPData(
            raw_data_fmp_dividends[:5], raw_data_fmp_stock_value[:5]
        )
        pre_alpha = pre.PreproccessingAlphavantageData(raw_data_alpha[:5])
    ...
    def construct(self):
        ...
        self.animate_from_to_with_description(
            json_mobject, pivot_anim, descr_text, transposed_df
        )
