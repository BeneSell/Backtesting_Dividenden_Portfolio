...
df = pd.DataFrame(data, columns=['Name', 'Category', 'Value'], index=["1", "2", "3", "4"])

class DataFrameTransformation(mn.Scene):
    def construct(self):
        
        text = df.to_json(orient='records', lines=True).replace("\"", "").replace("B", """<span foreground="red">B</span>""")
        ...
        self.wait(2)
        self.play(mn.Transform(json_mobject, pivot_anim), mn.Transform(descr_text, transposed_df))
        self.wait(2)
