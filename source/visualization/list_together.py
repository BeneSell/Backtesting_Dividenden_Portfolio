...
stability_data = [
    {"company": "Apple", "stability": "071.07"},
...
class DataFrameTransformation(mn.Scene):
    ...
    def construct(self):
        ...
        grouped_df["rank"] = (
            grouped_df["rank_growth"].astype(float)
            + grouped_df["rank_yield"].astype(float)
            + grouped_df["rank_stability"].astype(float)
        )

        grouped_df = grouped_df[["company", "rank"]].sort_values(
        ...
        grouped_df_animated.add(
            mn.SurroundingRectangle(grouped_df_animated.get_columns()[1])
        )

        # custom animations
        # pivot_anim.add(pivot_anim.get_cell((2,2), color=mn.RED))

        self.play(mn.Write(descr_text)), mn.Write(
            yield_animated.shift(mn.RIGHT * 4).scale(0.25)
        ), mn.Write(growth_animated.scale(0.25)), mn.Write(
            stability_animated.shift(mn.LEFT * 4.5).scale(0.25)
        )
        ...
        stability_animated.add(
            mn.SurroundingRectangle(stability_animated.get_columns()[2])
        )
        ...
        self.play(
            mn.FadeOut(mn.MarkupText("Sort them by rank").scale(0.8).shift(mn.UP * 1.5))
        )

        self.play(
            mn.Transform(
                yield_animated,
                self.create_table(yiled_df.sort_values(by="rank_yield"))
                .shift(mn.RIGHT * 4)
                .scale(0.25)
                .add(mn.SurroundingRectangle(yield_animated.get_columns()[2])),
            ),
            mn.Transform(
                growth_animated,
                self.create_table(growth_df.sort_values(by="rank_growth"))
                .scale(0.25)
                .add(mn.SurroundingRectangle(growth_animated.get_columns()[2])),
            ),
            mn.Transform(
                stability_animated,
                self.create_table(stability_df.sort_values(by="rank_stability"))
                .shift(mn.LEFT * 4.5)
                .scale(0.25)
                .add(mn.SurroundingRectangle(stability_animated.get_columns()[2])),
            ),
        )
        ...
        self.animate_from_to_with_description(
            stability_animated, grouped_df_animated, descr_text, first_df
        )

        # self.animate_from_to_with_description(json_mobject, pivot_anim, descr_text, transposed_df)

    def animate_from_to_with_description(
        self,
        from_mobject: mn.Mobject,
        to_mobject: mn.Mobject,
        start_description,
        new_description,
    ):

        self.play(
            mn.Transform(from_mobject, to_mobject),
