...
def setup():
    ...
    data_combined = [{'date': pd.to_datetime('2021-01-01').to_period("M"), "information": "adjusted close", "TEST": 2.0},
                     {'date': pd.to_datetime('2021-01-01').to_period("M"), "information": "dividend amount", "TEST": 2.0},
                     {'date': pd.to_datetime('2022-01-01').to_period("M"), "information": "adjusted close", "TEST": 5.0},
                     {'date': pd.to_datetime('2023-01-01').to_period("M"), "information": "adjusted close", "TEST": 10.0},
                     {'date': pd.to_datetime('2024-01-01').to_period("M"), "information": "adjusted close", "TEST": 11.0},
                     {'date': pd.to_datetime('2025-01-01').to_period("M"), "information": "adjusted close", "TEST": 12.0},
                     {'date': pd.to_datetime('2025-01-01').to_period("M"), "information": "dividend amount", "TEST": 3.0},
                     {'date': pd.to_datetime('2025-02-01').to_period("M"), "information": "dividend amount", "TEST": 4.0}]
...
def test_get_dividends(setup):
    ...
    # print(df_combined)
    # print(result["dividend amount"])
    assert result["dividend amount"].iloc[0] == 2.0
...
def test_compound_interest_calc_recursive_with_extras(setup):
    ...
    result = single_stock_checker.compound_interest_calc_recursive_with_extras(100, dividends["adjusted close"].count(), dividends["adjusted close"].count(),first_stock_price, dividends["dividend amount"], dividends["adjusted close"], output) 
    print(output)
