...
class StrategieExecution:
    ...
    def test_a_portfolio(
        ...
        middle_date: datetime.datetime = datetime.date(2015, 12, 31),
        ...
        args:
            ...
            middle_date (datetime.datetime, optional): the start date.
            look_forward_years (int, optional): how many years to look forward.
        """
        result = []
        ...
        for x in df_of_stocks["symbol"].to_list():
            ...
            temp = calc_data.check_money_made_by_div(
                middle_date, look_forward_years, x, self.combined_data, 100
            )
    ...
    def parallelize_check_along_time_method(
        self,
        x,
        start_date_of_iterations: datetime.datetime = datetime.date(1990, 12, 31),
        look_backward_years: int = 10,
        look_forward_years: int = 5,
    ) -> pd.DataFrame:
        """
        ...
        args:
            x (int): the time span
            start_date_of_iterations (datetime.datetime, optional): the start date.
        ...
        temp_list_of_stocks = self.check_all_stocks(
            start_date_of_iterations + timedelta(days=365 * x), look_backward_years
        )
        ...
        temp_df["time_span"] = x
        temp_df["start_date"] = start_date_of_iterations + timedelta(days=365 * x)
        temp_df["middle_date"] = (
            start_date_of_iterations
            + timedelta(days=365 * x)
            + timedelta(days=365 * look_backward_years)
    ...
    def check_along_time_axis(
        self,
        start_date_of_iterations: datetime.datetime = datetime.date(1985, 12, 31),
    ...
    def check_along_time_and_timespan(self):
        ...
        for x in range(1, 4):
            ...
            temp_result = self.check_along_time_axis(
                look_backward_years=11, look_forward_years=x
            )
